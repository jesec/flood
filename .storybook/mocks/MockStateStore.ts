...
import type {TransferSummary, TransferHistory} from '@shared/types/TransferData';
...
class MockStateStore {
  ...
  private computeTaxonomy(torrents: Record<string, TorrentProperties>): Taxonomy {
    ...
    if (!taxonomy.tagCounts.hasOwnProperty('untagged')) {
    ...
    Object.values(torrents).forEach((torrent) => {
      ...
      // Status counts - count each status separately
  ...
  setState(updates: Partial<MockState>): void {
    ...
    const {taxonomy, ...actualUpdates} = updates;
