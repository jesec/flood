...
const getDestination = async (
  services: import('fastify').FastifyRequest['services'],
...
const torrentsRoutes = async (router: FastifyInstance) => {
  /**
   * GET /api/torrents
  ...
  router.get('/', async (request, reply) =>
    request.services.torrentService
      .fetchTorrentList()
      .then((data) => {
        if (data == null) {
          throw new Error();
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.post<unknown, unknown, AddTorrentByURLOptions>('/add-urls', async (request, reply) => {
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.post<unknown, unknown, AddTorrentByFileOptions>('/add-files', async (request, reply) => {
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.post<unknown, unknown, CreateTorrentOptions>('/create', async (request, reply) => {
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.post<unknown, unknown, StartTorrentsOptions>('/start', async (request, reply) =>
    request.services.clientGatewayService.startTorrents(request.body).then(
      (response) => {
        request.services.torrentService.fetchTorrentList();
        return reply.status(200).send(response);
      },
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.post<unknown, unknown, StopTorrentsOptions>('/stop', async (request, reply) =>
    request.services.clientGatewayService.stopTorrents(request.body).then(
      (response) => {
        request.services.torrentService.fetchTorrentList();
        return reply.status(200).send(response);
      },
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.post<unknown, unknown, CheckTorrentsOptions>('/check-hash', async (request, reply) =>
    request.services.clientGatewayService.checkTorrents(request.body).then(
      (response) => {
        request.services.torrentService.fetchTorrentList();
        return reply.status(200).send(response);
      },
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.post<unknown, unknown, MoveTorrentsOptions>('/move', async (request, reply) => {
    let sanitizedPath: string | null = null;
    try {
    ...
    return request.services.clientGatewayService.moveTorrents({...request.body, destination: sanitizedPath}).then(
      (response) => {
        request.services.torrentService.fetchTorrentList();
        return reply.status(200).send(response);
      },
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.post<unknown, unknown, DeleteTorrentsOptions>('/delete', async (request, reply) =>
    request.services.clientGatewayService.removeTorrents(request.body).then(
      (response) => {
        request.services.torrentService.fetchTorrentList();
        return reply.status(200).send(response);
      },
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.post<unknown, unknown, ReannounceTorrentsOptions>('/reannounce', async (request, reply) => {
    ...
    return request.services.clientGatewayService.reannounceTorrents(parsedResult.data).then(
      (response) => {
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.patch<unknown, unknown, SetTorrentsInitialSeedingOptions>('/initial-seeding', async (request, reply) =>
    request.services.clientGatewayService.setTorrentsInitialSeeding(request.body).then(
      (response) => {
        request.services.torrentService.fetchTorrentList();
        return reply.status(200).send(response);
      },
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.patch<unknown, unknown, SetTorrentsPriorityOptions>('/priority', async (request, reply) =>
    request.services.clientGatewayService.setTorrentsPriority(request.body).then(
      (response) => {
        request.services.torrentService.fetchTorrentList();
        return reply.status(200).send(response);
      },
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.patch<unknown, unknown, SetTorrentsSequentialOptions>('/sequential', async (request, reply) =>
    request.services.clientGatewayService.setTorrentsSequential(request.body).then(
      (response) => {
        request.services.torrentService.fetchTorrentList();
        return reply.status(200).send(response);
      },
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.patch<unknown, unknown, SetTorrentsTagsOptions>('/tags', async (request, reply) => {
    ...
    return request.services.clientGatewayService.setTorrentsTags(parsedResult.data).then(
      (response) => {
        request.services.torrentService.fetchTorrentList();
        return reply.status(200).send(response);
      },
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.patch<unknown, unknown, SetTorrentsTrackersOptions>('/trackers', async (request, reply) =>
    request.services.clientGatewayService.setTorrentsTrackers(request.body).then(
      (response) => {
        request.services.torrentService.fetchTorrentList();
        return reply.status(200).send(response);
      },
  ...
  router.get<{hashes: string}>(
    ...
    async (request, reply) => {
      ...
      const archive = tar.pack(sessionDirectory, {
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.get('/:hash/contents', async (request, reply) =>
    request.services.clientGatewayService.getTorrentContents(request.params.hash).then(
      (contents) => reply.status(200).send(contents),
      ({code, message}) => reply.status(500).send({code, message}),
    ),
  );
  ...
  router.patch<{hash: string}, unknown, SetTorrentContentsPropertiesOptions>(
    '/:hash/contents',
    async (request, reply) =>
      request.services.clientGatewayService.setTorrentContentsPriority(request.params.hash, request.body).then(
      ...
      return request.services.clientGatewayService
        ...
        .then((contents) => {
          ...
          if (filePathsToDownload.length === 1) {
            ...
            reply.setHeader('content-disposition', contentDisposition(fileName, {type: 'inline'}));
          ...
          reply.attachment(`${selectedTorrent.name}.tar`);
          ...
          tarStream.pipe(reply).once('close', () => {
            tarStream.unpipe(reply);
            reply.destroy();
          });
   ...
   * @param {string} hash.path
   */
  router.get('/:hash/details', async (request, reply) => {
    try {
  ...
  router.get<{hash: string}>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        request.on('close', () => mediainfoProcess.kill('SIGTERM'));
   ...
   * @param {string} hash.path
   */
  router.get('/:hash/peers', async (request, reply) =>
    request.services.clientGatewayService.getTorrentPeers(request.params.hash).then(
      (peers) => reply.status(200).send(peers),
      ({code, message}) => reply.status(500).send({code, message}),
    ),
  );
   ...
   * @return {Error} 500 - failure response - application/json
   */
  router.get('/:hash/trackers', async (request, reply) =>
    request.services.clientGatewayService.getTorrentTrackers(request.params.hash).then(
      (trackers) => reply.status(200).send(trackers),
      ({code, message}) => reply.status(500).send({code, message}),
    ),
  );
};
