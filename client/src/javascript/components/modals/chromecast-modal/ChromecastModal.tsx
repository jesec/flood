...
import {Form, FormRow, Select, SelectItem, FormRowItem} from '@client/ui';
import {Start, Stop, Pause} from '@client/ui/icons';
import TorrentActions from '@client/actions/TorrentActions';
import UIStore from '@client/stores/UIStore';

import {getChromecastContentType, isFileChromecastable, isFileSubtitles} from '@shared/util/chromecastUtil';
...
const GenerateMagnetModal: FC = () => {
  ...
  const [selectedSubtitles, setSelectedSubtitles] = useState<number>(-1);
          ...
          <Form>
            ...
            <FormRow>
              <Select
                ...
                onSelect={(id) => setSelectedSubtitles(id as number)}>
                {subtitleSources.map((source) => (
                  <SelectItem key={source} id={source}>
                    {source === -1 ? i18n._('chromecast.modal.subtitle.none') : contents[source].filename}
                  </SelectItem>
                ))}
              </Select>
            </FormRow>
