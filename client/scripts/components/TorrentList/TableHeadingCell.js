...
class TableHeadingCell extends React.Component {
  ...
  handleMouseMove(event) {
    ...
    if (this.lastMouseX != null) {
      widthDelta = event.screenX - this.lastMouseX;
    }
    ...
    if (nextCellWidth > 20) {
      ...
      this.resizeLine.style.transform
        = `translateX(${Math.max(0, event.screenX - this.tableHeadingX + this.props.scrollOffset)}px)`;
  ...
  getHeadingElements() {
    ...
    return properties.map(({slug, label}) => {
      let handle = null;
